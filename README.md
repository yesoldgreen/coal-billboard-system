...
一个完整的煤矿内参告示牌管理和展示系统，包含官网宣传、后台管理和客户端展示功能。
...
- 根路径 `/` 会自动跳转到官网页面 `/site/`
- 官网用于宣传新的收费会员群，并引导用户扫码预约进群
- 页面采用响应式布局，适合手机和微信内嵌浏览器查看
- 二维码使用普通图片 `<img>` 展示，便于用户长按识别
...
官网静态页面位于 `public/site`，由后端通过 `/site` 路径托管，根路径 `/` 会重定向到官网。
